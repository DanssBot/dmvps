...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://www.dropbox.com/s/w3jtpqbezex4s70/blockBT.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} No se puede vincular a Github !" && exit 0
	wget https://www.dropbox.com/s/igwpabffpl7e3zc/blockBT.sh -O /etc/ger-frm/blockBT.sh &> /dev/null
...
echo -e "  Panel de Firewall By ADM-VPS 2021 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}"
msg -bar2
